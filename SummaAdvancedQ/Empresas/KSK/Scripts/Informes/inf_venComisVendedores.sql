...
/****** Object:  StoredProcedure [dbo].[inf_venComisVendedores]    Script Date: 6/10/2024 11:47:05 AM ******/

/*
exec [espinf_venComisVendedores] '2386', '2605', '20240501', '20240601',0,1
select * from venvendedores
drop procedure espinf_venComisVendedores

*/
/* IMPORTANTE: ESTE PROCEDIMIENTO SE ENCUENTRA MODIFICADO. EL STD SE ENCUENTRA EN CARPETA SUMMA ESTANDAR Y LA COPIA DEL MODIFICADO SE ENCUENTRA EN LA CARPETA DE LA EMPRESA KSK*/
...
as
--if exists (select * from tempdb..dbo.#SucComiTable)
if OBJECT_ID ('Tempdb..#SucComiTable') is not null
drop table #SucComiTable
create table #SucComiTable (Sucursal varchar(15)) 
...
if(@SobreCobrado=0)
	...
	insert #SucComiTable
	select 
	distinct genAsiSegmentos.Segmento2C
	from venMovimientos
	...
	INNER JOIN venClientes ON venMovimientos.Cliente_Id = venClientes.genEntidades
	where venMovimientos.venTipoMov in('F','R') and 
	venSubTipoMov.AfectaComVenta = 1 
--	and venVendedores.Vendedor_Id between @VendedorDesde and @VendedorHasta  -- saco el filtro de vendedor desde/hasta
	and conAsientos.Fecha BETWEEN @FechaDesde and @FechaHasta and 
	conAsientos.Empresa_Id = @Empresa_Id and conAsientos.Posteado=1 and conAsientos.Anulado=0
	and genAsiSegmentos.Segmento2C in (select Sucursal from #SucComiTable)
	ORDER BY 1,5
	END
else
	BEGIN
	insert #SucComiTable
	SELECT distinct A.Segmento2C
	FROM (SELECT genAsiSegmentos.Segmento2C,venVendedores.Vendedor_Id, venVendedores.Nombre, dbo.Segmento(Segmento_Id, Segmento1C, Segmento2C, Segmento3C, 
	Segmento4C, Segmento1N, Segmento2N, Segmento3N, Segmento4N) as Asiento, venClientes.RazonSocial, 
	...
	venMovimientosAP.Importe != 0 and conAsientos.Posteado = 1 and conAsientos.Anulado = 0) A 
	GROUP BY A.Segmento2C,A.Vendedor_Id, A.Nombre, A.Asiento, A.RazonSocial, A.Fecha, A.Porcentaje
	...
	where venMovimientos.venTipoMov = 'C' AND
	venSubTipoMov.AfectaComVenta = 1 
--	and venVendedores.Vendedor_Id between @VendedorDesde and @VendedorHasta   -- retiro el filtro de vendedor desde hasta
	and conAsientos.Fecha BETWEEN @FechaDesde and @FechaHasta and conAsientos.Empresa_Id=@Empresa_Id 
	and venMovimientosAP.Importe != 0 and conAsientos.Posteado = 1 and conAsientos.Anulado = 0
	and genAsiSegmentos.Segmento2C in (select Sucursal from #SucComiTable))
