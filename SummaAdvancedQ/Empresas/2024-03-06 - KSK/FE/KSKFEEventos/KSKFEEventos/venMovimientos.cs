    ...
    public class venMovimientos
    {
        ...
        public VentasCommon.venMovimientosConStockDS AsignarImportes(VentasCommon.venMovimientosConStockDS exch, VentasCommon.venMovimientosConStockDS exchNoPost, int pIndice, Framework.Core.Command cmd)
        {
            ...
            else
            {
                ...
                { Cartera_Id = ds.Identificadores[pIndice].Cartera_Id, Importe = ds.Principal[0].ImporteAAsignar });
                else
                    NetoCartera.Importe = ds.Principal[0].ImporteAAsignar;
            ...
            ds = CalcularImportes(ds, ds.Principal[0].ImporteAAsignar);
        ...
        public VentasCommon.venMovimientosConStockDS CobrarPorMercadoPagoOrden(VentasCommon.venMovimientosConStockDS pCommonDS, Command cmd)
        {
            ...
            Generalidades.Especializacion.UnirDatatSets(pCommonDS, ds);
            string Segmento1N;
            Segmento1N = DateTime.Now.ToString("yyyyMMddHHmmss");

            if (pCommonDS.Cobranza[0].Cartera_Id == "MPAGO")
            {
                string mensajeMP = string.Empty;
                
                

                KSKBE.MP.GenerarOrden mp = new KSKBE.MP.GenerarOrden(pCommonDS.Principal[0].Segmento2C);
                // ya tengo la sucursal y caja de mercado pago llenas en mp.mpSucursal y mp.mpCaja

                //string external_reference = pCommonDS.Principal[0].Segmento1C + pCommonDS.Principal[0].Segmento2C + pCommonDS.Principal[0].Segmento3C + "-" + Segmento1N;
                string external_reference = Segmento1N;
                KSKBE.MP.GenerarOrden MpOrden = new KSKBE.MP.GenerarOrden(pCommonDS.Cobranza[0].Importe, external_reference, mp.mpSucursal, mp.mpCaja);
            ...
            DataTable tabla = conectar.EjecutarConsulta($@"select Serie as Sucursal, 
                                RegPatronal as Caja from genSucursalesEmpr (nolock) where Sucursal = '{pCommonDS.Principal[0].Segmento2C}'");
            ...
            Console.WriteLine($"Antes de Detectar Escaneo del comprobante {Segmento1N} de la sucursal {mpsucursal} y de la caja {mpcaja}" );
            GenerarOrden MpOrdenEsc = new GenerarOrden(Segmento1N, mpsucursal, mpcaja);
            bool escaneado = MpOrdenEsc.Escaneo(Segmento1N).Result;
        ...
        public venMovimientosConStockDS ControlarAntesDeCobrar(venMovimientosConStockDS pCommonDS, Command cmd)
        {
            ...
            //22/09/2023 - Gustavo - Lo llamo desde el formulario justo antes de abrir la cobranza ya que en ese momento tengo el valor neto de la operacion
